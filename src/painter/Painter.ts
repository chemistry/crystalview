...
import type { ICamera } from '../camera/ICamera.js';
import { Marker } from '../molecule3d/Marker.js';
...
export class Painter {
  ...
  public uprojectToMarker(data: {
    ...
    const tx = data.x - this.context.getCenterX();
    const ty = data.y - this.context.getCenterY();
    const clickR = 9;
    ...
    for (const marker of molecule.markers) {
      ...
      if (Math.abs(projection.x - tx) < clickR && Math.abs(projection.y - ty) < clickR) {
        const length = Math.sqrt(
          (projection.x - tx) * (projection.x - tx) + (projection.y - ty) * (projection.y - ty)
        );
  ...
  private unitCellLine(context: CanvasContext, obj: ZBufferItem) {
    context.line(obj.x1!, obj.y1!, obj.x2!, obj.y2!, obj.color!);
  }

  private drawBondLine(context: CanvasContext, obj: ZBufferItem) {
    context.line(obj.x1!, obj.y1!, obj.x2!, obj.y2!, obj.color!);
  }

  private drawAtom(context: CanvasContext, obj: ZBufferItem) {
    context.circle(obj.x!, obj.y!, obj.r!, obj.color!);
  }

  private drawMarker(context: CanvasContext, obj: ZBufferItem) {
    context.circle(obj.x!, obj.y!, 3, '#ffda4d');
  }

  private drawMarkerContact(context: CanvasContext, obj: ZBufferItem) {
    context.linedashed(obj.x1!, obj.y1!, obj.x2!, obj.y2!, '#ffda4d', [4, 7]);
  }

  private drawLabel(context: CanvasContext, obj: ZBufferItem) {
