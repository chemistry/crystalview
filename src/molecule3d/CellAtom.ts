import { ChemElements, ChemElementInfo } from '@chemistry/elements';
...
export class CellAtom {
   ...
   * Element
   */
  public element: ChemElementInfo;
  ...
  constructor(data: {
    ...
    const element = ChemElements.getBySymbol(data.type);
    if (!element) {
  ...
  public hasChild(position: Vec3): boolean {
    for (const ch of this.children) {
      if (ch.fractional.equals(position)) {
        return true;
      }
    }
    return false;
   ...
   */
  public getChild(position: Vec3): Atom | null {
    for (const ch of this.children) {
      if (ch.fractional.equals(position)) {
        return ch;
      }
   ...
   */
  public getChieldWithSymetry(symetry: Matrix3x4): Atom | null {
    const predictedPostion = symetry.project(this.fractional);
