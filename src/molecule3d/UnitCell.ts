...
export class UnitCell {
  ...
  public beta: number;
  public gamma: number;
  public spaceGroup: SpaceGroupInfo;
  ...
  private init(): void {
    ...
    const v = Math.sqrt(
      1 -
        Math.cos(pAlpha) * Math.cos(pAlpha) -
        Math.cos(pBeta) * Math.cos(pBeta) -
        Math.cos(pGamma) * Math.cos(pGamma) +
        2 * Math.cos(pAlpha) * Math.cos(pBeta) * Math.cos(pGamma)
    );
    const cc = Math.sin(pGamma) / this.c / v;
    const alphaa = Math.acos(
      (Math.cos(pBeta) * Math.cos(pGamma) - Math.cos(pAlpha)) / Math.sin(pBeta) / Math.sin(pGamma)
    );
    ...
    //
    for (const symStr of this.spaceGroup.s) {
